...
SINGLE_BATTLE_TEST("Meloetta reverts to Aria Form upon switching out (Traits)")
{
    u32 trial = 0;
    ...
    GIVEN {
        PLAYER(SPECIES_MELOETTA_ARIA);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_HONCHKROW) { Ability(ABILITY_SUPER_LUCK); Innates(ABILITY_INSOMNIA); }
    } WHEN {
        TURN { MOVE(player, MOVE_RELIC_SONG); }
        TURN { 
            switch (trial)
