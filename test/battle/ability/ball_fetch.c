...
#if MAX_MON_ITEMS > 1
WILD_BATTLE_TEST("Ball Fetch causes the Pokémon to pick up the last failed Ball at the end of the turn (Items)")
{
    enum Item item = ITEM_NONE;
...
WILD_BATTLE_TEST("Ball Fetch only picks up the first failed ball, once per battle (Items)")
{
    enum Item item = ITEM_NONE;
    enum Item item2 = ITEM_NONE;
...
SINGLE_BATTLE_TEST("Ball Fetch doesn't trigger in Trainer Battles (Items)")
{
    enum Item item = ITEM_NONE;
