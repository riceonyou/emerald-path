...
SINGLE_BATTLE_TEST("Wind Power displays its message before fainting when triggered (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WATTREL) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_WIND_POWER); HP(1); Speed(1); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(10); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_AIR_CUTTER); }
    } SCENE {
...
DOUBLE_BATTLE_TEST("Tailwind does not trigger Wind Power on an absent ally battler (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TAILWIND) == EFFECT_TAILWIND);
        PLAYER(SPECIES_WOBBUFFET) { Speed(10); }
