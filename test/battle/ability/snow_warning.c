...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Snow Warning sets up hail for 8 turns with Icy Rock (Gen6-8) (Items)")
{
    GIVEN {
        WITH_CONFIG(B_SNOW_WARNING, GEN_8);
        WITH_CONFIG(B_ABILITY_WEATHER, GEN_6);
...
SINGLE_BATTLE_TEST("Snow Warning sets up snow for 8 turns with Icy Rock (Gen9+) (Items)")
{
    GIVEN {
        WITH_CONFIG(B_SNOW_WARNING, GEN_9);
        WITH_CONFIG(B_ABILITY_WEATHER, GEN_9);
