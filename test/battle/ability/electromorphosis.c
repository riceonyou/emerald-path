...
SINGLE_BATTLE_TEST("Electromorphosis displays its message before fainting when triggered (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_BELLIBOLT) { Ability(ABILITY_DAMP); Innates(ABILITY_ELECTROMORPHOSIS); HP(1); Speed(1); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(10); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_SCRATCH); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Electromorphosis triggers on each multistrike hit but Charge does not stack (Traits)")
{
    s16 dmgBefore, dmgAfterSingleHit, dmgAfterMultiHit;
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_SCRATCH) == DAMAGE_CATEGORY_PHYSICAL);
        ...
        ASSUME(GetMoveType(MOVE_THUNDER_SHOCK) == TYPE_ELECTRIC);
        PLAYER(SPECIES_BELLIBOLT) { Ability(ABILITY_DAMP); Innates(ABILITY_ELECTROMORPHOSIS); Speed(10); }
        OPPONENT(SPECIES_PERSIAN) { Ability(ABILITY_UNNERVE); Innates(ABILITY_LIMBER); Speed(20); MaxHP(500); HP(500); }
    } WHEN {
        TURN { MOVE(player, MOVE_THUNDER_SHOCK); }
        TURN { MOVE(opponent, MOVE_SCRATCH); }
        TURN { MOVE(player, MOVE_THUNDER_SHOCK); }
