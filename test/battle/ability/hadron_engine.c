...
#if MAX_MON_TRAITS > 1
SINGLE_BATTLE_TEST("Hadron Engine creates Electric Terrain when entering the battle (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_MIRAIDON) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_HADRON_ENGINE); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN {}
    } SCENE {
...
SINGLE_BATTLE_TEST("Hadron Engine boosts the Pokemon's Special Attack on Electric Terrain even if not grounded (Traits)", s16 damage)
{
    bool32 overrideTerrain, airBalloon;
    ...
    GIVEN {
        ...
        ASSUME(GetMoveEffect(MOVE_GRASSY_TERRAIN) == EFFECT_GRASSY_TERRAIN);
        ASSUME(GetMoveCategory(MOVE_POWER_GEM) == DAMAGE_CATEGORY_SPECIAL);
        PLAYER(SPECIES_MIRAIDON) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_HADRON_ENGINE); Moves(MOVE_POWER_GEM, MOVE_CELEBRATE); Item(airBalloon ? ITEM_AIR_BALLOON : ITEM_NONE); Speed(1); }
        OPPONENT(SPECIES_WOBBUFFET) { Moves(MOVE_GRASSY_TERRAIN, MOVE_CELEBRATE); Speed(2); }
    } WHEN {
        if (overrideTerrain)
            TURN { MOVE(opponent, MOVE_GRASSY_TERRAIN); }
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Hadron Engine boosts the Pokemon's Special Attack on Electric Terrain even if not grounded (Items)", s16 damage)
{
    bool32 overrideTerrain, airBalloon;
    ...
    GIVEN {
        ...
        ASSUME(GetMoveEffect(MOVE_GRASSY_TERRAIN) == EFFECT_GRASSY_TERRAIN);
        ASSUME(GetMoveCategory(MOVE_POWER_GEM) == DAMAGE_CATEGORY_SPECIAL);
        PLAYER(SPECIES_MIRAIDON) { Ability(ABILITY_HADRON_ENGINE); Moves(MOVE_POWER_GEM, MOVE_CELEBRATE); Items(ITEM_PECHA_BERRY, airBalloon ? ITEM_AIR_BALLOON : ITEM_NONE); Speed(1); }
        OPPONENT(SPECIES_WOBBUFFET) { Moves(MOVE_GRASSY_TERRAIN, MOVE_CELEBRATE); Speed(2); }
    } WHEN {
        if (overrideTerrain)
            TURN { MOVE(opponent, MOVE_GRASSY_TERRAIN); }
