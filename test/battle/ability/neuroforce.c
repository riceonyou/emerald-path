...
SINGLE_BATTLE_TEST("Neuroforce increases the strength of super-effective moves by 25% (Traits)", s16 damage)
{
    ...
    GIVEN {
        ASSUME(GetMoveType(MOVE_SHADOW_BALL) == TYPE_GHOST);
        ASSUME(GetMoveType(MOVE_SCRATCH) == TYPE_NORMAL);
        PLAYER(SPECIES_NECROZMA_DUSK_MANE) { Ability(ABILITY_LIGHT_METAL); Innates(ability); Item(ITEM_ULTRANECROZIUM_Z); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_CELEBRATE, gimmick: GIMMICK_ULTRA_BURST); }
        TURN { MOVE(player, move); }
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Neuroforce increases the strength of super-effective moves by 25% (Items)", s16 damage)
{
    enum Move move;
    enum Ability ability;
    PARAMETRIZE { ability = ABILITY_NEUROFORCE; move = MOVE_SHADOW_BALL; }
    ...
    GIVEN {
        ASSUME(GetMoveType(MOVE_SHADOW_BALL) == TYPE_GHOST);
        ASSUME(GetMoveType(MOVE_SCRATCH) == TYPE_NORMAL);
        PLAYER(SPECIES_NECROZMA_DUSK_MANE) { Ability(ability); Items(ITEM_PECHA_BERRY, ITEM_ULTRANECROZIUM_Z); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_CELEBRATE, gimmick: GIMMICK_ULTRA_BURST); }
        TURN { MOVE(player, move); }
