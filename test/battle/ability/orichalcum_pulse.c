...
SINGLE_BATTLE_TEST("Orichalcum Pulse boost applies even if the target holds Utility Umbrella (Items)", s16 damage)
{
    u16 targetItem;
    PARAMETRIZE { targetItem = ITEM_NONE; }
    PARAMETRIZE { targetItem = ITEM_UTILITY_UMBRELLA; }
...
SINGLE_BATTLE_TEST("Orichalcum Pulse does not boost physical moves if holder has Utility Umbrella (Items)", s16 damage)
{
    u16 holdItem;
    PARAMETRIZE { holdItem = ITEM_NONE; }
    PARAMETRIZE { holdItem = ITEM_UTILITY_UMBRELLA; }
