...
#if MAX_MON_TRAITS > 1
SINGLE_BATTLE_TEST("Heatproof halves damage from fire type moves (Traits)")
{
    s16 damage[2];
    GIVEN {
        PLAYER (SPECIES_WOBBUFFET);
        OPPONENT (SPECIES_BRONZONG) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_HEATPROOF); }
    }
    WHEN {
        TURN { MOVE(player, MOVE_EMBER); }
        TURN { MOVE(player, MOVE_WORRY_SEED); }
...
SINGLE_BATTLE_TEST("Heatproof halves the damage done by burn from 1/8th to 1/16th (Gen1-6) or 1/16th to 1/32nd (Gen 7+)")
{
    ...
    GIVEN {
        WITH_CONFIG(B_BURN_DAMAGE, config);
        PLAYER (SPECIES_BRONZONG) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_HEATPROOF); Status1(STATUS1_BURN); }
        OPPONENT (SPECIES_WOBBUFFET);
    }
    WHEN {
        TURN {}
