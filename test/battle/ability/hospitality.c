...
DOUBLE_BATTLE_TEST("Hospitality user restores 25% of ally's max HP after taking hazard damage (Traits)")
{
    u32 speedLeft, speedRight = 0;
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Speed(1); HP(1); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(speedLeft); };
        PLAYER(SPECIES_POLTCHAGEIST) { Speed(speedRight); Ability(ABILITY_HEATPROOF); Innates(ABILITY_HOSPITALITY); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(110); };
        OPPONENT(SPECIES_WOBBUFFET) { Speed(100); };
    } WHEN {
        TURN {
