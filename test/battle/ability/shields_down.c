...
WILD_BATTLE_TEST("Wild Minior appear in Meteor form without transforming (Traits)")// To be replaced with WILD_DOUBLE_BATTLE_TEST when that is made possible.
{
    GIVEN {
        PLAYER(SPECIES_MINIOR_CORE) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_SHIELDS_DOWN); }
...
SINGLE_BATTLE_TEST("Trainers' Minior appear in Core form (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) 
        OPPONENT(SPECIES_MINIOR_METEOR) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_SHIELDS_DOWN); }
    } WHEN {
        TURN {}
    } SCENE {
        ABILITY_POPUP(opponent, ABILITY_SHIELDS_DOWN);
