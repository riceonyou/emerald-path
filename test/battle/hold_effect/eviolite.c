...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Eviolite boosts Defense and Sp. Def for unevolved Pokemon (Items)", s16 damage)
{
    u16 move;
    u32 item;
...
SINGLE_BATTLE_TEST("Eviolite does not boost Defense or Sp. Def for evolved Pokemon (Items)", s16 damage)
{
    u16 move;
    u32 item;
...
SINGLE_BATTLE_TEST("Eviolite uses original species after Transform (Items)", s16 damage)
{
    u32 item;
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TRANSFORM) == EFFECT_TRANSFORM);
        PLAYER(SPECIES_PIKACHU) { Items(ITEM_PECHA_BERRY, item); Moves(MOVE_TRANSFORM, MOVE_CELEBRATE); }
        OPPONENT(SPECIES_GYARADOS) { Moves(MOVE_SCRATCH, MOVE_CELEBRATE); }
    } WHEN {
        TURN { MOVE(player, MOVE_TRANSFORM); MOVE(opponent, MOVE_CELEBRATE); }
        TURN { MOVE(player, MOVE_CELEBRATE); MOVE(opponent, MOVE_SCRATCH); }
