...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Life Orb activates when users attack is succesful (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Life Orb activates if it hits a Substitute (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_LIFE_ORB); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Life Orb does not activate if using status move on a Substitute (Items)")
{
    GIVEN {
        ASSUME(MoveIgnoresSubstitute(MOVE_GROWL));
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_LIFE_ORB); }
...
SINGLE_BATTLE_TEST("Life Orb does not activate if using a status move (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_LIFE_ORB); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Life Orb doesn't cause any HP loss if user is unable to attack (Items)")
{
    PASSES_RANDOMLY(25, 100, RNG_PARALYSIS);
    GIVEN {
...
SINGLE_BATTLE_TEST("Life Orb does not activate if on a confusion hit (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_LIFE_ORB); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Life Orb does not activate if move was absorbed by target (Items)")
{
    GIVEN {
        WITH_CONFIG(B_REDIRECT_ABILITY_IMMUNITY, GEN_5);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_LIFE_ORB); }
...
SINGLE_BATTLE_TEST("Life Orb activates if move connected but no damage was dealt (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_LIFE_ORB); }
        OPPONENT(SPECIES_WOBBUFFET) { HP(1); }
...
SINGLE_BATTLE_TEST("Life Orb does not activate on a charge turn (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_LIFE_ORB); }
        OPPONENT(SPECIES_WOBBUFFET);
...
DOUBLE_BATTLE_TEST("Life Orb activates if damage was inflicted by a dancer move through the dancer ability (Items)")
{
    GIVEN {
        ASSUME(IsDanceMove(MOVE_FIERY_DANCE));
        ASSUME(GetMoveEffect(MOVE_HEAL_BELL) == EFFECT_HEAL_BELL);
        PLAYER(SPECIES_ORICORIO) { Speed(200); Ability(ABILITY_DANCER); Items(ITEM_PECHA_BERRY, ITEM_LIFE_ORB); Status1(STATUS1_SLEEP); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(3); }
        OPPONENT(SPECIES_WYNAUT) { Speed(2); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(1); }
    } WHEN {
