...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Metronome Item gradually boosts power of consecutively used moves by 20%, up to 100% (Items)")
{
    s16 damage[METRONOME_TURNS];
    u32 j;
    GIVEN {
...
SINGLE_BATTLE_TEST("Metronome Item's boost is reset if the attacker uses a different move (Items)")
{
    s16 damage[2];
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_METRONOME); }
...
SINGLE_BATTLE_TEST("Metronome Item's boost is reset if the move fails (Items)")
{
    s16 damage[2];
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_METRONOME); }
...
SINGLE_BATTLE_TEST("Metronome Item counts called moves instead of the calling move (Items)")
{
    s16 damage[2];
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_METRONOME); }
...
SINGLE_BATTLE_TEST("Metronome Item counts charging turn of moves for its attacking turn (Items)", s16 damage)
{
    enum Item item;
...
SINGLE_BATTLE_TEST("Metronome Item doesn't increase damage per hit of multi-hit moves (Items)")
{
    s16 damage[3];
    GIVEN {
        ASSUME(IsMultiHitMove(MOVE_FURY_ATTACK));
