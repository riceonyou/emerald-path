...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Protective Pads protected moves still make direct contact (Items)", s16 damage)
{
    enum Ability ability;
    PARAMETRIZE { ability = ABILITY_KLUTZ; }
    PARAMETRIZE { ability = ABILITY_FLUFFY; }
...
SINGLE_BATTLE_TEST("Protective Pads doesn't reduce tough claws damage (Items)", s16 damage)
{
    enum Item item;
    PARAMETRIZE { item = ITEM_NONE; }
    PARAMETRIZE { item = ITEM_PROTECTIVE_PADS; }
...
SINGLE_BATTLE_TEST("Protective Pads doesn't invalid unseen fist (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Protective Pads protects from Rocly Helmet Damage (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_PROTECTIVE_PADS); }
...
SINGLE_BATTLE_TEST("Protective Pads protects from Protect's secondary effects (Items)")
{
    enum Move move;
