...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Leftovers recovers 1/16th HP at end of turn (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Leftovers does nothing if max HP (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Leftovers does nothing if Heal Block applies (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { MaxHP(100); HP(1); Items(ITEM_PECHA_BERRY, ITEM_LEFTOVERS); }
        OPPONENT(SPECIES_WOBBUFFET);
