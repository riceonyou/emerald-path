...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Shed Shell allows switching out even when trapped by Mean Look (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Shed Shell allows switching out even when trapped by Shadow Tag (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_SHED_SHELL); }
        PLAYER(SPECIES_WYNAUT);
...
SINGLE_BATTLE_TEST("Shed Shell allows switching out even when trapped by Arena Trap (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Shed Shell does not allow Teleport when trapped (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TELEPORT) == EFFECT_TELEPORT);
