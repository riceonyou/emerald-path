...
#if MAX_MON_ITEMS > 1
#define HITS 5
SINGLE_BATTLE_TEST("Shell Bell recovers 1/8 of HP from after the last hit from all hits of a multi hit move (Items)")
{
    s16 multiHitDamage[HITS];
    s16 totalDamage = 0;
    s16 shellBellRecovery = 0;
...
SINGLE_BATTLE_TEST("Shell Bell recovers no HP if the move did no damage (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(1); Items(ITEM_PECHA_BERRY, ITEM_SHELL_BELL); }
        OPPONENT(SPECIES_WOBBUFFET) { HP(1); };
...
SINGLE_BATTLE_TEST("Shell Bell activates if it hits a Substitute (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(1); Items(ITEM_PECHA_BERRY, ITEM_SHELL_BELL); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Shell Bell activates after Absorb (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(1); Items(ITEM_PECHA_BERRY, ITEM_SHELL_BELL); }
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Shell Bell activates after Rough Skin (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Shell Bell does not activate on Future Sight if the original user is on the field (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(1); Items(ITEM_PECHA_BERRY, ITEM_SHELL_BELL); }
        OPPONENT(SPECIES_WYNAUT);
...
SINGLE_BATTLE_TEST("Shell Bell restores 1/8 HP of damage dealt (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Shell Bell doesn't restore HP for damage dealt by a foreseen move (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FUTURE_SIGHT) == EFFECT_FUTURE_SIGHT);
        PLAYER(SPECIES_WOBBUFFET) { Level(16); Items(ITEM_PECHA_BERRY, ITEM_SHELL_BELL); HP(10); }
...
SINGLE_BATTLE_TEST("Shell Bell does not activate on Future Sight if the original user is not on the field (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WYNAUT);
        PLAYER(SPECIES_WOBBUFFET) { HP(1); Items(ITEM_PECHA_BERRY, ITEM_SHELL_BELL); }
...
DOUBLE_BATTLE_TEST("Shell Bell heals accumulated damage for spread moves (Items)")
{
    s16 opponentLeftDamage;
    s16 opponentRightDamage;
    s16 playerRightDamage;
...
SINGLE_BATTLE_TEST("Shell Bell restores 1/8 HP at move end, one strike (Items)")
{
    const u16 maxHp = 200;
    u16 hp, opponentHp;
    u16 hpGainFromDamage, hpGainActual;
...
SINGLE_BATTLE_TEST("Shell Bell recovers only 1 damage if the move only did 1 damage (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { HP(1); Items(ITEM_PECHA_BERRY, ITEM_SHELL_BELL); }
        OPPONENT(SPECIES_WOBBUFFET) { HP(1); };
...
TO_DO_BATTLE_TEST("If a Pokémon steals a Shell Bell with Thief or Covet, it will recover HP for the use of that move that stole the Shell Bell (Items)")
TO_DO_BATTLE_TEST("If a Pokémon steals a Shell Bell with Magician, it will recover HP for the use of that move that stole the Shell Bell (Items)")
#endif
