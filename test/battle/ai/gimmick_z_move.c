...
#if MAX_MON_ITEMS > 1
AI_SINGLE_BATTLE_TEST("AI uses Z-Moves. (Items)")
{
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT );
        ASSUME(GetMoveType(MOVE_QUICK_ATTACK) == TYPE_NORMAL);
...
AI_SINGLE_BATTLE_TEST("AI uses Z-Moves -- conserves Z-move if target will faint anyway. (Items)")
{
    u32 currentHP;
    PARAMETRIZE { currentHP = 1; }
    PARAMETRIZE { currentHP = 500; }
...
AI_SINGLE_BATTLE_TEST("AI uses Z-Moves to ensure a low-accuracy KO. (Items)")
{
    u32 species, ability;
    PARAMETRIZE { species = SPECIES_TORKOAL; ability = ABILITY_DROUGHT; }
    PARAMETRIZE { species = SPECIES_PELIPPER; ability = ABILITY_DRIZZLE; }
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT );
        PLAYER(species) { Ability(ability); HP(1); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_ELECTRIUM_Z); Moves(MOVE_THUNDER); }
    } WHEN {
    if (ability == ABILITY_DROUGHT)
        TURN { EXPECT_MOVE(opponent, MOVE_THUNDER, gimmick: GIMMICK_Z_MOVE); }
    else
...
AI_SINGLE_BATTLE_TEST("AI uses Z-Moves -- Extreme Evoboost (Items)")
{
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT );
        ASSUME(GetMoveType(MOVE_QUICK_ATTACK) == TYPE_NORMAL);
...
AI_SINGLE_BATTLE_TEST("AI uses Z-Moves to bypass move limitations (Items)")
{
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT );
        ASSUME(GetMoveType(MOVE_QUICK_ATTACK) == TYPE_NORMAL);
...
AI_SINGLE_BATTLE_TEST("AI uses Z-Moves -- 10,000,000 Volt Thunderbolt (Items)")
{
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT );
        PLAYER(SPECIES_WOBBUFFET);
...
AI_SINGLE_BATTLE_TEST("AI uses Z-Moves -- Z-Conversion (Items)")
{
    enum Ability ability;
    PARAMETRIZE { ability = ABILITY_NONE; }
    PARAMETRIZE { ability = ABILITY_OPPORTUNIST; }
...
AI_SINGLE_BATTLE_TEST("AI uses Z-Moves -- Z-Destiny Bond is not used in singles (Items)")
{
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT );
        PLAYER(SPECIES_WOBBUFFET);
...
AI_DOUBLE_BATTLE_TEST("AI uses Z-Moves -- Z-Destiny Bond is used when about to die (Items)")
{
    u32 currentHP;
    PARAMETRIZE { currentHP = 1; }
    PARAMETRIZE { currentHP = 500; }
...
AI_SINGLE_BATTLE_TEST("AI uses Z-Moves -- Z-Detect (Items)")
{
    enum Move move;
    PARAMETRIZE { move = MOVE_THUNDERBOLT; }
    PARAMETRIZE { move = MOVE_CLOSE_COMBAT; }
...
AI_SINGLE_BATTLE_TEST("AI uses Z-Moves -- Z-Happy Hour (Items)")
{
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT );
        ASSUME(GetMoveType(MOVE_QUICK_ATTACK) == TYPE_NORMAL);
...
TO_DO_BATTLE_TEST("TODO: AI uses Z-Moves -- Z-Haze (Items)")

TO_DO_BATTLE_TEST("TODO: AI uses Z-Moves -- Z-Mirror Move (Items)")

AI_SINGLE_BATTLE_TEST("AI uses Z-Moves -- Z-Nature Power (Items)")
{
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT | AI_FLAG_PREDICT_MOVE);
        PLAYER(SPECIES_WOBBUFFET);
...
// Requires handling for Wish passing/Healing Wish/other ways to determine what pokemon to heal via switching into.
TO_DO_BATTLE_TEST("TODO: AI uses Z-Moves -- Z-Parting Shot (Items)")

AI_SINGLE_BATTLE_TEST("AI uses Z-Moves -- Z-Splash (Items)")
{
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT );
        ASSUME(GetMoveType(MOVE_QUICK_ATTACK) == TYPE_NORMAL);
...
TO_DO_BATTLE_TEST("TODO: AI uses Z-Moves -- Z-Tailwind (Items)")

AI_SINGLE_BATTLE_TEST("AI uses Z-Moves -- Z-Transform (Items)")
{
    u32 currentHP, move;
    PARAMETRIZE { currentHP = 1;   move = MOVE_HEADBUTT; }
    PARAMETRIZE { currentHP = 1;   move = MOVE_THUNDERBOLT; }
...
TO_DO_BATTLE_TEST("TODO: AI uses Z-Moves -- Z-Trick Room (Items)")
#endif
