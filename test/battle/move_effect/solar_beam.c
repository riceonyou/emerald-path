...
SINGLE_BATTLE_TEST("Solar Beam does half damage if Sandstorm is up (Gen3+) (Traits)", s16 damage)
{
    enum Ability ability;
    u32 config;
    ...
    GIVEN {
        WITH_CONFIG(B_SANDSTORM_SOLAR_BEAM, config);
        PLAYER(SPECIES_HIPPOWDON) { Ability(ABILITY_LIGHT_METAL); Innates(ability); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_SOLAR_BEAM); }
        TURN { SKIP_TURN(player); }
