...
SINGLE_BATTLE_TEST("Trick can be used against targets with an active form change that doesn't require items (Items)")
{
    GIVEN {
        PLAYER(SPECIES_XERNEAS);
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_HP_GOLD_BOTTLE_CAP, ITEM_ORAN_BERRY); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_TRICK); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_TRICK, opponent);
...
SINGLE_BATTLE_TEST("Trick does not remove the user's choice lock if both the target and use are holding choice items before Gen 5 (Items)")
{
    GIVEN {
        WITH_CONFIG(B_MODERN_TRICK_CHOICE_LOCK, GEN_4);
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_HP_GOLD_BOTTLE_CAP, ITEM_CHOICE_SCARF); MovesWithPP({MOVE_TRICK, 1}, {MOVE_CELEBRATE, 10}); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_HP_GOLD_BOTTLE_CAP, ITEM_CHOICE_SCARF); }
    }
    WHEN {
        TURN { MOVE(player, MOVE_TRICK); }
        TURN { FORCED_MOVE(player); }
