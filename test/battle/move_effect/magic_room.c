...
#if MAX_MON_ITEMS > 1
DOUBLE_BATTLE_TEST("Magic Room prevents item hold effects (Items)")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_BERRY_JUICE].holdEffect == HOLD_EFFECT_RESTORE_HP);
        ASSUME(GetMoveEffect(MOVE_DRAGON_RAGE) == EFFECT_FIXED_HP_DAMAGE);
...
SINGLE_BATTLE_TEST("Magic Room: An item that can activate will activate once Magic Room is over (Items)")
{
    GIVEN {
