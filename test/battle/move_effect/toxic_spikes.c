...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Toxic Spikes do not poison airborne Pokemon (Items)")
{
    u32 species = SPECIES_WOBBUFFET;
    enum Item item = ITEM_NONE;
    enum Move move1 = MOVE_CELEBRATE;
...
SINGLE_BATTLE_TEST("Toxic Spikes are removed by grounded Poison-type Pokémon on switch in (Items)")
{
    u32 species;
    enum Item item = ITEM_NONE;
    enum Move move = MOVE_CELEBRATE;
...
//  Tested in Gen 7 on cartridge
SINGLE_BATTLE_TEST("Toxic Spikes are not removed by Poison-type Pokémon affected by Magnet Rise on switch in (Items)")
{
    GIVEN {
        ASSUME(GetSpeciesType(SPECIES_EKANS, 0) == TYPE_POISON);
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Toxic Spikes inflicts poison on switch in after Primal Reversed mon fainted (Items)") // Oddly specific, but encountered during testing
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_MEMENTO) == EFFECT_MEMENTO); // Faints the user.
        PLAYER(SPECIES_WOBBUFFET) {Speed(5); }
