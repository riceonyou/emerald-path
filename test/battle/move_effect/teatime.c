...
DOUBLE_BATTLE_TEST("Teatime causes all Pokémon to consume their berry (Items)")
{
    ...
    PARAMETRIZE { user = opponentRight; }
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Items(ITEM_HP_GOLD_BOTTLE_CAP, ITEM_LIECHI_BERRY); }
        PLAYER(SPECIES_WYNAUT) { Items(ITEM_HP_GOLD_BOTTLE_CAP, ITEM_LIECHI_BERRY); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_HP_GOLD_BOTTLE_CAP, ITEM_LIECHI_BERRY); }
        OPPONENT(SPECIES_WYNAUT) { Items(ITEM_HP_GOLD_BOTTLE_CAP, ITEM_LIECHI_BERRY); }
    } WHEN {
        TURN { MOVE(user, MOVE_TEATIME); }
    } SCENE {
        if (user == playerLeft) {
