...
#if MAX_MON_TRAITS > 1
DOUBLE_BATTLE_TEST("Perish Song can be absorbed and will not affect those that absorbed it (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_LANTURN) { HP(1); Ability(ABILITY_WATER_ABSORB); Innates(ABILITY_VOLT_ABSORB); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_VOLTORB) { Ability(ABILITY_AFTERMATH); Innates(ABILITY_SOUNDPROOF); }
    } WHEN {
        TURN {
            MOVE(opponentLeft, MOVE_ELECTRIFY, target: playerLeft);
            MOVE(playerLeft, MOVE_PERISH_SONG);
...
DOUBLE_BATTLE_TEST("Perish Song will be blocked by users Soundproof in gen3 (Traits)")
{
    u32 config = GEN_3;
    ...
    GIVEN {
        WITH_CONFIG(B_CHECK_USER_FAILURE, config);
        PLAYER(SPECIES_VOLTORB) { Ability(ABILITY_AFTERMATH); Innates(ABILITY_SOUNDPROOF); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
